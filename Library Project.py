database = {}
admin_database = {"ClaytonADM": "ssrules!"}
...
class Library:
  ...
  def show_books(self):
    print("Here are the current books in the library:\n"
          "Make sure to use the ISBN number when checking out!\n")
...
class Member:
  ...
  def new_member():
    print(
        "Please enter the following information to register with this library."
    )
    ...
    mmembership = 1
    mbooks = 0
    new_member = Member(mname, musername, memail, mpassword, mmembership,
                        mbooks)
...
def check_admin(admin_name, admin_password):
  for admin_check in admin_database:
...
def display_menu(username):
  ...
  while selection == 0:
    ...
    print("1. View Books in Library \n"
          ...
          "3. View Members \n"
          "4. Sign Out \n")
    choice = int(input("Choose what you would like (1, 2, 3, 4): "))
    if choice == 1:
      lib.show_books()
    if choice == 2:
      lib.checkout_book()
...
book3 = Book("Harry Potter and the Sorcerers Stone", "J. K. Rowling", 124344,
             1997, 0)
book4 = Book("Harry Potter and the Chamber of Secrets", "J. K. Rowling",
             124345, 1998, 0)
book5 = Book("Harry Potter and the Prisoner of Azkaban", "J. K. Rowling",
             124346, 1999, 0)
